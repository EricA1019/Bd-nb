...
gd-run:
	@echo "Launching Godot (GUI)…"
	"$(GODOT)" --path "$(PROJ)"

gd-headless:
	@echo "Launching Godot (headless)…"
	"$(GODOT)" --headless --path "$(PROJ)"

gut-all:
	@echo "Running all GUT tests (headless)…"
	"$(GODOT)" --headless --path "$(PROJ)" -s res://addons/gut/gut_cmdln.gd -gdir=res://scenes/tests -ginclude_subdirs -gprefix=test_ -gexit

# Usage: make gut-fast GARGS='-gselect=unit' or '-gsuite=My Suite'
gut-fast:
